...
def train_cohort_classifier(train_df: pl.DataFrame, lgbm_params: dict, cat_cols: list[str], target_map: dict):
    y = train_df.with_columns(pl.col("cohort").replace(target_map).cast(pl.Int64)).select("cohort").to_pandas()
